...
then
	...
	echo stamp is $stamp
	mv /core /core$stamp
	cp /tmp/appDemo_stdout /tmp/fw_app_stdout$stamp
	cp /tmp/stackbacktrace.txt /tmp/stackbacktrace$stamp.txt
	gzip /core$stamp
	gzip /tmp/fw_app_stdout$stamp
	gzip /tmp/stackbacktrace$stamp.txt
	tftp -p -l /core$stamp.gz -r core$stamp.gz $ipaddr
	tftp -p -l /tmp/fw_app_stdout$stamp.gz -r fw_app_stdout$stamp.gz $ipaddr
	...
	cp /tmp/fw_app_stdout$stamp.gz /var/log/$ridx/
