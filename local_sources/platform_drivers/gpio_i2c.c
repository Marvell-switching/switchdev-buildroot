...
static int gpio_i2c_read(struct gpio_i2c_chip *chip, u8 reg)
{
	...
	while (retry) {
		status = i2c_smbus_read_byte_data(chip->i2c, reg);
...
static int gpio_i2c_write(struct gpio_i2c_chip *chip, u8 reg, u8 value)
{
	...
	while (retry) {
		status = i2c_smbus_write_byte_data(chip->i2c, reg, value);
		if (unlikely(status < 0)) {
			msleep(GPIO_I2C_RETRY_INTERVAL);
			retry--;
			continue;
...
static int
gpio_i2c_reg_parse(struct gpio_i2c_chip *chip, struct device_node *np)
{
	struct gpio_i2c_reg *gpio_reg;
...
static const struct of_device_id of_gpio_i2c_match[] = {
	{ .compatible = "gpio-i2c" },
	{ }
};
MODULE_DEVICE_TABLE(of, of_gpio_i2c_match);

static const struct i2c_device_id gpio_i2c_id[] = {
	{ "gpio-i2c", 0 },
	{ }
};
MODULE_DEVICE_TABLE(i2c, gpio_i2c_id);

static struct i2c_driver gpio_i2c_driver = {
	.driver = {
		.of_match_table = of_match_ptr(of_gpio_i2c_match),
		.name  = "gpio-i2c",
		.owner = THIS_MODULE,
	},
	.probe = gpio_i2c_probe,
	.remove = gpio_i2c_remove,
	.id_table = gpio_i2c_id,
};
